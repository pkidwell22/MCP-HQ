...
final class MCPHTTPProbeTests: XCTestCase {
    ...
    func testProbeSendsConfiguredRemoteHeaders() throws {
        let server = try LocalMCPHTTPTestServer(scriptName: "header-protected-mcp.py", body: """
        import json
        from http.server import BaseHTTPRequestHandler, HTTPServer
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                if self.headers.get("Authorization") != "Bearer expected-secret":
                ...
                method = request.get("method")
                if method == "initialize":
                    self.send_json({"jsonrpc": "2.0", "id": request["id"], "result": {"protocolVersion": "2024-11-05", "capabilities": {"tools": {}}}})
                elif method == "notifications/initialized":
                    self.send_response(202)
                    self.send_header("Content-Length", "0")
                    self.end_headers()
                    ...
                    self.send_json({"jsonrpc": "2.0", "id": request["id"], "result": {"tools": [{"name": "authed-tool"}]}})
        ...
        let result = MCPHTTPProbe(
            timeout: 2,
            processEnvironment: ["MCP_AUTH_HEADER": "Bearer expected-secret"]
        ).probe(server: definition)

        XCTAssertEqual(result.status, .healthy)
        XCTAssertEqual(result.toolNames, ["authed-tool"])
        XCTAssertEqual(result.pingSucceeded, true)
    }
    ...
    func testProbeTimesOutWithInitializePhaseMessage() throws {
        let server = try LocalMCPHTTPTestServer(scriptName: "timeout-init-mcp.py", body: """
        import json
        import time
        from http.server import BaseHTTPRequestHandler, HTTPServer
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                ...
                if request.get("method") == "initialize":
                    ...
                    response = {"jsonrpc": "2.0", "id": request.get("id"), "error": {"code": -32000, "message": "not reached"}}
                payload = json.dumps(response).encode()
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.send_header("Content-Length", str(len(payload)))
        ...
        XCTAssertEqual(result.status, .error)
        XCTAssertTrue(result.message.contains("Timed out while waiting for MCP HTTP initialize response from"), result.message)
        XCTAssertTrue(result.message.contains("127.0.0.1"), result.message)
    }
